...
abstract class TaskRunner extends Thread {
  ...
  public TaskRunner(TaskTracker.TaskInProgress tip, TaskTracker tracker, 
    ...
    ArrayList<String> validDirsList = new ArrayList<String>();
    String[] inputLocalDirs = conf.getLocalDirs();
    //this.localdirs = conf.getLocalDirs();

    for (int i = 0; i < inputLocalDirs.length; i ++ ) {
      try {
        DiskChecker.checkDir(new File(inputLocalDirs[i]));
        validDirsList.add(inputLocalDirs[i]);
      } catch (Throwable e) {
        
      }
    }
    this.localdirs = validDirsList.toArray(new String[0]);
