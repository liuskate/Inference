...
public class MRAdmin extends Configured implements Tool {
  ...
  private int printLiveTaskTrackerNum() throws IOException {
    Configuration conf = getConf();
    
    // Create the client
    JobTrackerProtocol jobTrackerProtocol = 
      (JobTrackerProtocol) 
      RPC.getProxy(JobTrackerProtocol.class, 
                   JobTrackerProtocol.versionID, 
                   JobTracker.getAddress(conf), getUGI(conf), conf,
                   NetUtils.getSocketFactory(conf, JobTrackerProtocol.class));

    System.out.println(jobTrackerProtocol.activeTaskTrackersSize());
    
    return 0;
  }

  private int printBlackTaskTrackerNum() throws IOException {
    Configuration conf = getConf();
    ...
    System.out.println(jobTrackerProtocol.blackTaskTrackersSize());
    
    return 0;
  }

  private int printTaskTrackerNum() throws IOException {
    Configuration conf = getConf();
    ...
    System.out.println(jobTrackerProtocol.taskTrackersSize());
    
    return 0;
  }

  private int printSogoureport() throws IOException {
    System.out.print("mapred_all=");
    printTaskTrackerNum();
    System.out.print("mapred_livenode=");
    printLiveTaskTrackerNum();
    System.out.print("mapred_deadnode=");
    printBlackTaskTrackerNum();
    return 0;
  }

  private int updateUser() throws IOException {
    Configuration conf = getConf();
    // Create the client
    JobTrackerProtocol jobTrackerProtocol = 
      (JobTrackerProtocol) 
  ...
  public int run(String[] args) throws Exception {
    ...
    try {
      if ("-updateuser".equals(cmd)) {
        ...
        exitCode = printSogoureport();
      } else if ("-mapred_all".equals(cmd)) {
        exitCode = printTaskTrackerNum();
      } else if ("-mapred_deadnode".equals(cmd)) {
        exitCode = printBlackTaskTrackerNum();
      } else if ("-mapred_livenode".equals(cmd)) {
        exitCode = printLiveTaskTrackerNum();
