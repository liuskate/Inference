...
class ReduceTask extends Task {
  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    class ShuffleRamManager implements RamManager {
      ...
      throws InterruptedException {
        ...
        while ((size + requestedSize) > maxSize) {
          LOG.info("ReduceTask.reserver triggers required size bigger than maxSize, requested size is " + requestedSize +
            " original size is " + size + " current free heap is " + Runtime.getRuntime().freeMemory());
          // Close the input...
          if (in != null) {
            try {
              in.close();
      ...
      boolean canFitInMemory(long requestedSize) {
        ...
        Class<? extends MemoryCalculatorPlugin> clazz =
          ((JobConf)conf).getClass(TaskTracker.MAPRED_TASKTRACKER_MEMORY_CALCULATOR_PLUGIN_PROPERTY,
        ...
        //LOG.info(" canFitInMemory Using memoryCalculator : " + memoryCalculatorPlugin);

        return (requestedSize < Integer.MAX_VALUE && 
                requestedSize < maxSingleShuffleLimit &&
                requestedSize < (long)(memoryCalculatorPlugin.getFreeMemorySize() * 
                  MAX_SINGLE_SHUFFLE_SEGMENT_FRACTION) &&
                requestedSize < Runtime.getRuntime().freeMemory());
      }
    }
    ...
    private class MapOutputCopier extends Thread {
      ...
      throws IOException, InterruptedException {
        ...
        if (shuffleInMemory) {
          ...
          try {
            ...
            // If problem happens when do the shuffling, we should also fall back to do the shuffle in disk
      ...
      throws IOException {
        ...
        try {
          ...
          LOG.info("shuffleToDisk failed to shuffle from " + mapOutputLoc.getTaskAttemptId(), 
                   ioe);
          ...
          try {
            ...
            LOG.info("shuffleToDisk failed to discard map-output from " + 
                mapOutputLoc.getTaskAttemptId(), ignored);
          }
          mapOutput = null;
    ...
    public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf,
      ...
      this.solidMaxBackoff = conf.getInt("mapred.reduce.copy.solidbackoff", 0);
        ...
        while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
          ...
          while (numInFlight > 0 && mergeThrowable == null) {
            ...
            if (cr.getSuccess()) {  // a successful copy
              ...
              // sogou special modifications, using the fix backOff maximum value
              if ((solidMaxBackoff > 0) && (solidMaxBackoff < currentBackOff)) {
                currentBackOff = solidMaxBackoff;
              }
