...
public class JobLocalizer {
  ...
  private static final short SETGID = (short)1512; //2750 permission
  ...
  public void createLocalDirs() throws IOException {
    ...
    for (Path localDir : localDirs) {
      ...
      try {
        ...
        LOG.warn("Unable to create the user directory : " + localDir + " Exception: " + ioe);
        continue;
      }
      userDirStatus = true;
    }
  ...
  public void createLocalDirsWith2750() throws IOException {
    boolean userDirStatus = false;
    // create all directories as rwxr-s---
    for (Path localDir : localDirs) {
      // create $mapred.local.dir/taskTracker/$user
      if (!lfs.mkdirs(localDir)) {
        LOG.warn("Unable to create the user directory : " + localDir);
        continue;
      }
      if (lfs instanceof RawLocalFileSystem) {
        ((RawLocalFileSystem)lfs).setRawPermision(localDir,(short)SETGID);
      }
      userDirStatus = true;
    }
    if (!userDirStatus) {
  ...
  public void createUserDirs() throws IOException {
    ...
    for (Path localDir : localDirs) {
      ...
      try {
        ...
        LOG.warn("Unable to create job cache directory : " + jobDir + " IOException: " + ioe);
        //Ignore
      }
      ...
      try {
        ...
        LOG.warn("Unable to create distributed-cache directory : " + distDir + " IOException: " + ioe);
        //Ignore
      }
    }
  ...
  public void createUserDirsWith2750() throws IOException {
    LOG.info("Initializing user " + user + " on this TT.");
    ...
    for (Path localDir : localDirs) {
      ...
      final Path jobDir =
        new Path(localDir, TaskTracker.JOBCACHE);
      if (!lfs.mkdirs(jobDir)) {
        LOG.warn("Unable to create job cache directory : " + jobDir);
      } else {
        jobCacheDirStatus = true;
        if (lfs instanceof RawLocalFileSystem) {
          ((RawLocalFileSystem)lfs).setRawPermision(jobDir,(short)SETGID);
        }
      }
      ...
      final Path distDir =
        new Path(localDir, TaskTracker.DISTCACHEDIR);
      if (!lfs.mkdirs(distDir)) {
        LOG.warn("Unable to create distributed-cache directory : " + distDir);
      } else {
        distributedCacheDirStatus = true;
        if (lfs instanceof RawLocalFileSystem) {
          ((RawLocalFileSystem)lfs).setRawPermision(distDir,(short)SETGID);
        }
      }
    }
    if (!jobCacheDirStatus) {
  ...
  public void createJobDirs() throws IOException {
    ...
    for (Path localDir : localDirs) {
      ...
      try {
        ...
        LOG.warn("Not able to create job directory " + fullJobDir.toString() + " IOException: " + ioe);
      }
    }
    if (!initJobDirStatus) {
  ...
  public void createJobDirsWith2750() throws IOException {
    boolean initJobDirStatus = false;
    for (Path localDir : localDirs) {
      Path fullJobDir = new Path(localDir, JOBDIR);
      if (lfs.exists(fullJobDir)) {
      ...
      // create $mapred.local.dir/taskTracker/$user/jobcache/$jobid
      if (!lfs.mkdirs(fullJobDir)) {
        LOG.warn("Not able to create job directory " + fullJobDir.toString());
      } else {
        initJobDirStatus = true;
        if (lfs instanceof RawLocalFileSystem) {
          ((RawLocalFileSystem)lfs).setRawPermision(fullJobDir,(short)SETGID);
        }
      }
    }
    if (!initJobDirStatus) {
  ...
  public void initializeJobLogDirWith2750() throws IOException {
    Path jobUserLogDir = new Path(TaskLog.getJobDir(jobid).toURI().toString());
    if (!lfs.mkdirs(jobUserLogDir)) {
      throw new IOException(
          "Could not create job user log directory: " + jobUserLogDir);
    }
    if (lfs instanceof RawLocalFileSystem) {
      ((RawLocalFileSystem)lfs).setRawPermision(jobUserLogDir,(short)SETGID);
    }
  }
  ...
  private void localizeJobJarFile(JobConf localJobConf) throws IOException {
    ...
    long jarFileSize = -1;
    boolean preBindJob = localJobConf.getBoolean(JobClient.JOBCLIENT_PRE_BIND_JOB, false);
    if ((!preBindJob) && (jarFile != null)) {
  ...
  public void createWorkDirWith2750(JobConf jConf) throws IOException {
    // create $mapred.local.dir/taskTracker/$user/jobcache/$jobid/work
    final Path workDir = lDirAlloc.getLocalPathForWrite(WORKDIR, ttConf);
    if (!lfs.mkdirs(workDir)) {
      throw new IOException("Mkdirs failed to create "
    ...
    if (lfs instanceof RawLocalFileSystem) {
      ((RawLocalFileSystem)lfs).setRawPermision(workDir,(short)SETGID);
    }
