...
class SchedulingAlgorithms {
  ...
  public static class TunnelSupportedComparator extends FairShareComparator {
    private int maximumTunnelNum;
    public TunnelSupportedComparator(int maximumTunnelNum) {
      this.maximumTunnelNum = maximumTunnelNum;
    }
    @Override
    public int compare(Schedulable s1, Schedulable s2) {
      if ((s1 instanceof JobSchedulable) && (s2 instanceof JobSchedulable)) {
        boolean s1IsTunnel = ((JobSchedulable)s1).getJob().isTunnelJob();
        boolean s2IsTunnel = ((JobSchedulable)s2).getJob().isTunnelJob();
        if ((s1IsTunnel) && (!s2IsTunnel) && (s1.getRunningTasks() < maximumTunnelNum)) {
          return -1;
        } else if ((!s1IsTunnel) && (s2IsTunnel) && (s2.getRunningTasks() < maximumTunnelNum)) {
          return 1;
        }
      }
