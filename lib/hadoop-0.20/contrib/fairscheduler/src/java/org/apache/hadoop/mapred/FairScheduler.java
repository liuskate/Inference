...
public class FairScheduler extends TaskScheduler {
  ...
  protected boolean enablePreemptTasksForMemory;
  protected int eachTimeKilledForMemory;
  protected int memoryUsagePercentageThreshold;
  protected int generalTTMemoryUsageThreshold;

  protected Map<PoolSchedulable, Integer> starvedTunnelTasks = 
    new HashMap<PoolSchedulable, Integer>();
  ...
  public void start() {
    try {
      ...
      enablePreemptTasksForMemory = conf.getBoolean(
          "mapred.fairscheduler.preemptionformemory", false);
      eachTimeKilledForMemory = conf.getInt(
          "mapred.fairscheduler.eachtimekilledformemory", 1);
      memoryUsagePercentageThreshold = conf.getInt(
          "mapred.fairscheduler.memoryusagethreshold", 98);
      generalTTMemoryUsageThreshold = conf.getInt(
          "mapred.local.memoryusagethreshold", 85);
  ...
  private void updatePreemptionVariables() {
    ...
    for (TaskType type: MAP_AND_REDUCE) {
      for (PoolSchedulable sched: getPoolSchedulables(type)) {
        ...
        if (!isStarvedForInterMinShare(sched)) {
          sched.setLastTimeAtInterMinShare(now);
        }
  ...
  boolean isStarvedForInterMinShare(PoolSchedulable sched) {
    return (sched.starvedTunnelJob() > 0);
  }
    ...
    synchronized (taskTrackerManager) {
      synchronized (this) {
        for (TaskType type: MAP_AND_REDUCE) {
          ...
          tasksToPreempt = 0;
          for (PoolSchedulable sched: scheds) {
            target = tasksToPreemptForTunnel(sched, curTime);
            starvedTunnelTasks.put(sched, new Integer(target));
            tasksToPreempt += target;
          }

          if (tasksToPreempt > 0) {
            if (!onlyLogPreemption) {
              preemptTasksForTunnel(scheds);
            }
          }

          if (enablePreemptTasksForMemory) {
            preemptTasksForMemory(scheds);
          }
        }
      }
    }
  ...
  /**
   * Preempt task for the memory using threshold.
   */
  private void preemptTasksForMemory(List<PoolSchedulable> scheds) {
    ...
    Hashtable adjustTaskTracker = new Hashtable();
    for (TaskStatus status: runningTasks) {
      if (status.getMemoryUsage() > memoryUsagePercentageThreshold) {
        String taskTracker = status.getTaskTracker();
        Integer killedTask = (Integer)adjustTaskTracker.get(taskTracker);
        if (((killedTask == null) && (eachTimeKilledForMemory>= 1)) ||
          (killedTask.intValue() < eachTimeKilledForMemory)){
          try {
            LOG.info("TaskTracker " + taskTracker + " memory will be used up, kill a task running on it.");
            taskTrackerManager.killTask(status.getTaskID(), false);
          } catch (IOException e) {
            LOG.error("For memory usage, Failed to kill task " + status.getTaskID(), e);
          }

          int newKilledTask = killedTask.intValue() + 1;
          Integer newKilledTaskNum = new Integer(newKilledTask);
          adjustTaskTracker.put(taskTracker, newKilledTaskNum);
        }
      }
    }
  }
  ...
  private void preemptTasksForTunnel(List<PoolSchedulable> scheds) {
    if (scheds.isEmpty() || starvedTunnelTasks.isEmpty())
      return;

    TaskType taskType = scheds.get(0).getTaskType();

    // Collect running non tunnel tasks of this pool
    List<TaskStatus> runningTasks = new ArrayList<TaskStatus>();
    for (PoolSchedulable sched: scheds) {
      Integer starvedNumInteger = (Integer)starvedTunnelTasks.remove(sched);
      if (starvedNumInteger == null) {
        continue;
      }
      int starvedNum = starvedNumInteger.intValue();
      if (starvedNum == 0) {
        continue;
      }

      for (JobSchedulable js: sched.getJobSchedulables()) {
        if (!(js.getJob().isTunnelJob())) {
          runningTasks.addAll(getRunningTasks(js.getJob(), taskType));
        }
      }
      // Sort tasks into reverse order of start time
  ...
  /**
  * Count how many tasks of a given type the pool needs to preempt for the tunnel task.
  * If the pool has new coming tunnel task creating for at least twice of its preemption
  * timeout, it should preempt the difference between tunnel task current share and
  * inter min share.
  */
  protected int tasksToPreemptForTunnel(PoolSchedulable sched, long curTime) {
    String pool = sched.getName();
    long interMinShareTimeout = poolMgr.getInterMinSharePreemptionTimeout(pool);
    int tasksToPreempt = 0;

    if (curTime - sched.getLastTimeAtInterMinShare() > interMinShareTimeout) {
      tasksToPreempt = sched.starvedTunnelJob();// - sched.getRunningTunnelTasks();
    }

    if (tasksToPreempt > 0) {
      String message = "Should preempt " + tasksToPreempt + " " 
        + sched.getTaskType() + " tasks for tunnel job, pool " + sched.getName();
      eventLog.log("INFO", message);
      LOG.info(message);
    }
  ...
  public void setEnablePreemptForMemory(boolean enable) {
    this.enablePreemptTasksForMemory = enable;
  }

  public void setKillTaskForMemory(int killedForMemory) {
    this.eachTimeKilledForMemory = killedForMemory;
  }
  public void setMemoryUsageThreshold(int memoryUsage) {
    this.memoryUsagePercentageThreshold = memoryUsage;
  }
  public void setTTMemoryUsageThreshold(int threshold) {
    this.generalTTMemoryUsageThreshold = threshold;
    if (taskTrackerManager instanceof JobTracker) {
      ((JobTracker)taskTrackerManager).setTTMemoryThreshold(generalTTMemoryUsageThreshold);
    }
  }
}
