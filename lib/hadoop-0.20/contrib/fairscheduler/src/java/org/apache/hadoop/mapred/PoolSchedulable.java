...
public class PoolSchedulable extends Schedulable {
  ...
  public PoolSchedulable(FairScheduler scheduler, Pool pool, TaskType type) {
    ...
    this.lastTimeAtInterMinShare = currentTime;
  ...
  public int getInterMinShare() {
    return poolMgr.getInterMinShare(pool.getName());
  }
  ...
  public int getRunningTunnelTasks() {
    int ret = 0;
    if (poolMgr.getInterMinShare(pool.getName()) > 0) {
      for (JobSchedulable sched: jobScheds) {
        if (sched.getJob().isTunnelJob()) {
          ret += sched.getRunningTasks();
        }
      }
    }
  ...
  public int starvedTunnelJob() {
    int ret = 0;
    if (poolMgr.getInterMinShare(pool.getName()) > 0) {
      for (JobSchedulable sched: jobScheds) {
        if (sched.getJob().isTunnelJob()) {
          int target = Math.min(poolMgr.getInterMinShare(pool.getName()), sched.getDemand()) 
            - sched.getRunningTasks();
          ret = target > 0 ? ret + target : ret;
        }
      }
    }
    return ret;
  ...
  public Task assignTask(TaskTrackerStatus tts, long currentTime,
    ...
    if (poolMgr.getInterMinShare(pool.getName()) > 0) {
      comparator = new SchedulingAlgorithms.TunnelSupportedComparator(poolMgr.getInterMinShare(pool.getName()));
    } else if (mode == SchedulingMode.FIFO) {
  ...
  public long getLastTimeAtInterMinShare() {
    return lastTimeAtInterMinShare;
  }

  public void setLastTimeAtInterMinShare(long lastTimeAtInterMinShare) {
    this.lastTimeAtInterMinShare = lastTimeAtInterMinShare;
  }
