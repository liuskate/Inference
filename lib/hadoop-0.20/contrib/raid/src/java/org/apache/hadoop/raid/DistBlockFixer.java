...
public class DistBlockFixer extends BlockFixer {
  ...
  public void run() {
    while (running) {
      ...
      if (now - ticketRenewTime > ticketRenewInterval) {
        try {
          securityLogin(conf);
        } catch (Exception e) {
          System.err.println("[Error] Exception happen in DistBlockFixer run " + e);
        }
        ticketRenewTime = now;
      }
  ...
  public static void initNameNodeProtocol(Configuration conf) {
    try {
      NamenodeProtocol namenodeProtocol = createNamenode(conf);
      ExportedBlockKeys keys = namenodeProtocol.getBlockKeys();
      ...
      blockTokenSecretManager = new BlockTokenSecretManager(false,
        blockKeyUpdateInterval, blockTokenLifetime);
      blockTokenSecretManager.setKeys(keys);
    } catch (Exception e) {
      LOG.error("[Error] DistBlockFixer initNameNodeProtocol fail " + e);
      e.printStackTrace();
    }
  }

  protected static void loginAsNameNodeKeyTab(Configuration conf) throws IOException {
    if (UserGroupInformation.isSecurityEnabled()) {
      String raidBlockfixerKeyTabPath = conf.get("dfs.namenode.keytab.file", "/usr/lib/hadoop/conf/hdfs.keytab");
      String raidBlockfixerUserName = conf.get("dfs.namenode.kerberos.principal", "hdfs");
      raidBlockfixerUserName = SecurityUtil.getServerPrincipal(raidBlockfixerUserName, null);
  ...
  static class DistBlockFixerMapper
    ...
    protected void setup(Context context
      ...
      loginAsNameNodeKeyTab(conf);
      initNameNodeProtocol(conf);
    }
  }
