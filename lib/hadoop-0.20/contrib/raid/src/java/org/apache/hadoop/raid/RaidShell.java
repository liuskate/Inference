...
public class RaidShell extends Configured implements Tool {
  ...
  public RaidShell(Configuration conf) throws IOException {
    ...
    if (UserGroupInformation.isSecurityEnabled()) {
      String raidKeyTabPath = conf.get("raid.shell.keytab.file", "/usr/lib/hadoop/conf/hdfs.keytab");
      String raidUserName = conf.get("raid.shell.user.name", "hdfs");

      UserGroupInformation.loginUserFromKeytab(raidUserName, raidKeyTabPath);
    }
  }
  ...
  /* list all files under the directory <i>src</i>
    ...
    */
  private void travelAndFix(BlockFixer.BlockFixerHelper fixer, FileStatus src, FileSystem srcFs)
    throws IOException {
    ...
    if (items == null) {
      ...
      for (int i = 0; i < items.length; i++) {
        try {
          ...
          if (stat.isDir()) {
            travelAndFix(fixer, stat, srcFs);
          } else {
            Path fixedFilePath = new Path(cur.toUri().getPath());
            fixer.fixFile(fixedFilePath);
          }
        } catch (Exception e) {
          //Exception happened when fix one file, ignore and continue
          System.err.println("[Warn] Exception happens when fix file." + e);
        }
      }
      return;
    }
  ...
  public void recoverBlocks(String[] args, int startIndex)
    ...
    for (int i = startIndex; i < args.length; i++) {
      ...
      FileSystem srcFs = pathObj.getFileSystem(this.getConf());
      FileStatus[] srcs = srcFs.globStatus(pathObj);

      for(int j = 0; j < srcs.length; j ++) {
        travelAndFix(fixer, srcs[j], srcFs);
      }
    }
  }
