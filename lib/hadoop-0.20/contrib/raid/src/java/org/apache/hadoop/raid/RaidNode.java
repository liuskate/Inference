...
public abstract class RaidNode implements RaidProtocol {
  ...
  static private Path getOriginalParityFile(Path destPathPrefix, Path srcPath) {
    Path relativePath = makeRelative(srcPath);
    if (relativePath == null) {
      return destPathPrefix;
    }
    return new Path(destPathPrefix, relativePath);
  }
  ...
  public static RaidNode createRaidNode(String argv[], Configuration conf)
    ...
    if (UserGroupInformation.isSecurityEnabled()) {
      String raidKeyTabPath = conf.get("raid.keytab.file", "/usr/lib/hadoop/conf/raid.keytab");
      String raidUserName = conf.get("raid.user.name", "raid");
