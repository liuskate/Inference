...
import org.apache.hadoop.security.UserGroupInformation.ConfigFileCredentials;
...
class ConnectionHeader implements Writable {
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    if (ugiUsernamePresent) {
      ...
      boolean passwordPresent = in.readBoolean();
      if ((realUserNamePresent) && (!passwordPresent)) {
      ...
      } else if ((!realUserNamePresent) && (!passwordPresent)) {
        ugi = UserGroupInformation.createRemoteUser(username);
      } else if ((!realUserNamePresent) && passwordPresent) {
        String password = in.readUTF();
        ugi = UserGroupInformation.createConfigFileUser(username,password);
      }
    } else {
      ugi = null;
    }
  ...
  public void write(DataOutput out) throws IOException {
    ...
    if (ugi != null) {
      if (authMethod == AuthMethod.KERBEROS) {
        ...
        out.writeBoolean(false);
      } else if (authMethod == AuthMethod.CONFIGFILE) {
        //Send user and password for simple auth
        out.writeBoolean(true);
        out.writeUTF(ugi.getUserName());
        out.writeBoolean(false);
        String password = ugi.getPassword();
        if (password != null) {
          out.writeBoolean(true);
