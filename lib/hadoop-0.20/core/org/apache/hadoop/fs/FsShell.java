...
public class FsShell extends Configured implements Tool {
  ...
  private void printHelp(String cmd) {
    ...
    String snapshot = "-snapshot <path>: Make snapshot for the file/directory at <path>\n" +
    "\t\tinto the SNAPSHOT space, usually the '/snapshot' dir. \n" +
    "\t\t \n";
  ...
  private void snapshot(String string) throws IOException{
    Path f = new Path(string);
    FileSystem fs = f.getFileSystem(getConf());
    fs.snapshot(f);
    System.out.println("Snapshot operation succ for " + string);
    }
  ...
  private static void printUsage(String cmd) {
    ...
    if ("-fs".equals(cmd)) {
      ...
      System.err.println("           [-snapshot <path>]");
    ...
    if ("-put".equals(cmd) || "-test".equals(cmd) ||
      ...
      if (argv.length < 3) {
        ...
        return exitCode;
               ...
               "-text".equals(cmd) || "-snapshot".equals(cmd)) {
      if (argv.length < 2) {
        printUsage(cmd);
        return exitCode;
      }
