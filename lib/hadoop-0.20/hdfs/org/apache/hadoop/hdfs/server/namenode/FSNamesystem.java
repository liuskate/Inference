...
public class FSNamesystem implements FSConstants, FSNamesystemMBean,
  ...
  synchronized Collection<CorruptFileBlockInfo> listCorruptFileBlocks(String path,
    ...
    BlockIterator blkIterator = neededReplications.iterator(UnderReplicatedBlocks.QUEUE_WITH_CORRUPT_BLOCKS);
  ...
  /**
   *  make snapshot for specific file or directory.
   *  any existing snapshot file/dir will be removed first. if any error occurs,
   *  removed data might not be roll back.
   *  In current implementation, the files underconstruction are ignored,
   *  which should be improved in future. 
 * @param src
 * @return
 * @throws Exception
 */
  synchronized boolean snapshot(String src, FsPermission masked,
      String holder, String clientMachine) throws IOException {
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug("DIR* NameSystem.makeSnapshot: "
          + src);
    }
    if (isInSafeMode())
      throw new SafeModeException("Cannot snapshot " + src, safeMode);

    src = dir.normalizePath(src);
    boolean ret = false;
    try {
      INode[] dirs = dir.getExistingPathINodes(src);
      INode lastNode = dirs[dirs.length - 1];
      PermissionStatus permissions = new PermissionStatus(
          UserGroupInformation.getCurrentUser().getShortUserName(),
          null, masked);

      ret = snapshotInternal(src, lastNode, permissions, holder,
          clientMachine);
    } finally {
      if (!ret) {
        this.delete(SnapshotManager.getSnapshotName(src), true);
      }
    }
    return ret;
  }

  boolean snapshotInternal(String fullName, INode lastNode,
      PermissionStatus permissions, String holder, String clientMachine)
      throws IOException {
    String snapshot = SnapshotManager.getSnapshotName(fullName);
    if (lastNode.isDirectory()) {
      delete(snapshot, true);
      mkdirs(snapshot, permissions);
      INodeDirectory f = (INodeDirectory) lastNode;
      for (INode n : f.getChildren()) {
        snapshotInternal(
            fullName + Path.SEPARATOR_CHAR + n.getLocalName(), n,
            permissions, holder, clientMachine);
      }
    } else {
      // currently, we just skip files under construction
      if(lastNode.isUnderConstruction())
        return true;
      INodeFile f = (INodeFile) lastNode;
      // permission should be cloned.
      // startFile may apply umask to it
      PermissionStatus copyPermissions = new PermissionStatus(
          permissions.getUserName(), permissions.getGroupName(),
          new FsPermission(permissions.getPermission()));

      // create empty snapshot file.
      // in current implementation, create an empty file will not generate
      // blocks,
      // so it's clear.
      startFile(snapshot, copyPermissions, holder, clientMachine, true,
          f.getReplication(), f.getPreferredBlockSize());
      completeFile(snapshot, holder);
      INode[] dirs = dir.getExistingPathINodes(snapshot);
      INodeFile t = (INodeFile) dirs[dirs.length - 1];
      // new file and old files are snapshots to each other
      t.snapshot = f;
      f.snapshot = t;
      // copy blocks
      t.blocks = f.blocks;
