...
public class JspHelper {
  ...
  public void streamBlockInAscii(InetSocketAddress addr, long blockId, 
               Token<BlockTokenIdentifier> accessToken, long genStamp, 
                                 ...
                                 JspWriter out,
                                 Configuration conf, String charSet) 
    throws IOException {
  ...
  public static UserGroupInformation getDefaultWebUserWithPassword(Configuration conf
                                                       ) throws IOException {
    String[] strings = conf.getStrings(JspHelper.WEB_UGI_PROPERTY_NAME);
    if (strings == null || strings.length == 0) {
      throw new IOException("Cannot determine UGI from request or conf");
    }
    return UserGroupInformation.createConfigFileUser(strings[0], strings[1]);
  }
  ...
  public static UserGroupInformation getUGI(HttpServletRequest request,
    ...
    } else if (UserGroupInformation.isConfigFileEnabled()) { // configfile way enabled
      String user = request.getParameter("ugi");

      if(user == null) { // not specified in request
        ugi = getDefaultWebUserWithPassword(conf);
      } else {
        String[] ugiArray = user.split(",");
        ugi = UserGroupInformation.createConfigFileUser(ugiArray[0], ugiArray[1]);
      }
      ugi.setAuthenticationMethod(AuthenticationMethod.CONFIGFILE);
