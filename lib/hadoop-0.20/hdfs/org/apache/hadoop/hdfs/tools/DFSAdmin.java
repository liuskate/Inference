...
public class DFSAdmin extends FsShell {
  ...
  public void getAllNodeNum() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      DatanodeInfo[] dataNode = dfs.getClient().datanodeReport(DatanodeReportType.ALL);
      System.out.println(dataNode.length);
    } else {
      System.out.println("0");
    }
  }

  public void getDeadNodeNum() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
  ...
  public void getLiveNodeNum() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      ...
      System.out.println(live.length);
    } else {
      System.out.println("0");
    }
  }

  public void getDFSRemaining() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      ...
      System.out.println(100 - (used * 100 / presentCapacity));
    }
  }

  public void getDFSUsed() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      FsStatus ds = dfs.getStatus();
      long used = ds.getUsed();
      long remaining = ds.getRemaining();
      long presentCapacity = used + remaining;
      System.out.println(used * 100 / presentCapacity);
    }
  }

  public void getMissingBlock() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      System.out.println(dfs.getMissingBlocksCount());
    }
  }

  /* Build a NamenodeProtocol connection to the namenode and
   * set up the retry policy */ 
  protected NamenodeProtocol createNamenode()
    throws IOException {
  ...
  public void getImageState() throws IOException {
    NamenodeProtocol nameNode = createNamenode();
    int ret = nameNode.getFSImageState().get();
    System.out.println(ret);
  }

  public void getRackList() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      DatanodeInfo[] live = dfs.getClient().datanodeReport(DatanodeReportType.LIVE);
      for (DatanodeInfo dn : live) {
        System.out.println(dn.getHost()+"="+dn.getNetworkLocation());
      }
    } 
  }

  public void getBrokenDisk() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      DistributedFileSystem dfs = (DistributedFileSystem) fs;
      String[] brokenDiskList = dfs.getClient().getBrokenDiskList();
      if (brokenDiskList == null) {
        System.out.println("All disk healthy.");
        return;
      }
      boolean hasResult = false;
      for (String brokenDisk : brokenDiskList) {
        if ((brokenDisk != null) && (brokenDisk.length() > 0)) {
          System.out.println(brokenDisk);
          hasResult = true;
        }
      }
      if (!hasResult) {
  ...
  public void sogouReport() throws IOException {
    System.out.print("hdfs_all=");
    getAllNodeNum();
    System.out.print("hdfs_livenode=");
    getLiveNodeNum();
    System.out.print("hdfs_deadnode=");
    getDeadNodeNum();
    System.out.print("hdfs_used=");
    getDFSUsed();
    System.out.print("hdfs_remaining=");
    getDFSRemaining();
    System.out.print("hdfs_missingblock=");
    getMissingBlock();
    System.out.print("hdfs_image_status=");
    getImageState();
    System.out.println("hdfs_rack_list:");
    getRackList();
    System.out.println("hdfs_broken_disk:");
    getBrokenDisk();
  }
  ...
  public int run(String[] argv) throws Exception {
    ...
    try {
      if ("-updateuser".equals(cmd)) {
        updateUser();
      } else if ("-hdfs_rack_list".equals(cmd)) {
        getRackList();
      } else if ("-hdfs_image_status".equals(cmd)) {
        getImageState();
      } else if ("-hdfs_missingblock".equals(cmd)) {
        getMissingBlock();
      } else if ("-hdfs_remaining".equals(cmd)) {
        getDFSRemaining();
      } else if ("-hdfs_used".equals(cmd)) {
        getDFSUsed();
      } else if ("-hdfs_all".equals(cmd)) {
        getAllNodeNum();
      } else if ("-hdfs_deadnode".equals(cmd)) {
        getDeadNodeNum();
      } else if ("-hdfs_livenode".equals(cmd)) {
        getLiveNodeNum();
      } else if ("-hdfs_broken_disk".equals(cmd)) {
        getBrokenDisk();
      } else if ("-sogoureport".equals(cmd)) {
        sogouReport();
      } else if ("-report".equals(cmd)) {
        report();
      } else if ("-safemode".equals(cmd)) {
