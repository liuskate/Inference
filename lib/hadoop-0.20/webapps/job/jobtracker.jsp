...
<%!
  ...
  public void generateSummaryTable(JspWriter out, ClusterMetrics metrics,
    ...
    out.print("<table border=\"1\" cellpadding=\"5\" cellspacing=\"0\">\n"+
              ...
              "<th>Avg. Mem Usage</th>" +
              "<th>Avg. Cpu Usage</th>" +
              "<th>Avg. Map Slot Usage</th>" +
              "<th>Map Slot Free Rate (<80%)</th>" +
              "<th>Avg. Reduce Slot Usage</th>" +
              "<th>Reduce Slot Free Rate (<80%)</th>" +
    ...
    out.print("<tr><td>" + metrics.getRunningMaps() + "</td><td>" +
              ...
              "</td><td>" + tasksPerNode +
              "</td><td>" + metrics.getAvarageMemoryUsage() + "%" +
              "</td><td>" + metrics.getAvarageCpuUsage() + "%" +
              "</td><td>" + metrics.getAvarageMapSlotUsage() + "%" +
              "</td><td>" + metrics.getMapSlotFreeRate() + "%" +
              "</td><td>" + metrics.getAvarageReduceSlotUsage() + "%" +
              "</td><td>" + metrics.getReduceSlotFreeRate() + "%" +
...
<% 
  ...
  String showFoxPara = request.getParameter("showfox");
  boolean showFox = "truetrue".equals(showFoxPara);
%>
...
<%
  out.print(JSPUtil.generateJobTable("Running", runningJobs, 30, 0, tracker.conf, showFox));
%>
<hr>
...
if (completedJobs.size() > 0) {
  ...
  out.print(JSPUtil.generateJobTable("Completed", completedJobs, 0, 
    runningJobs.size(), tracker.conf, showFox));
  out.print("<hr>");
}
%>
...
if (failedJobs.size() > 0) {
  ...
  out.print(JSPUtil.generateJobTable("Failed", failedJobs, 0, 
    (runningJobs.size()+completedJobs.size()), tracker.conf, showFox));
  out.print("<hr>");
}
%>
...
<%
  ...
  (runningJobs.size()+completedJobs.size()+failedJobs.size()), showFox));
%>
<hr>
