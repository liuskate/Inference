...
public class ReservedInference extends Configured implements Tool{
	
	public static class ReservedInferenceMapper extends Mapper<Object, Text, Text, Text>{
		...
		protected void setup(Context context) throws IOException,
			...
			conf.set("inference.phase.name", "ReservedInferenceMapper");
			try {
	...
	public static class ReservedInferenceReducer extends Reducer<Text, Text, Text, Text>{
		...
		protected void setup(Context context)
				throws IOException, InterruptedException {
			Configuration conf = context.getConfiguration();
			conf.set("inference.phase.name", "ReservedInferenceReducer");
			try {
				im = new InferenceManager(conf);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public int run(String[] args) throws Exception {
		...
		try{
			...
			helpformat.printHelp("ReservedInference", opts);
			e.printStackTrace();
			System.exit(1);
		}
		Configuration conf = this.getConf();
		Job job = new Job(conf, "ReservedInference");
		job.setJarByClass(ReservedInference.class);
		job.setMapperClass(ReservedInferenceMapper.class);
		job.setReducerClass(ReservedInferenceReducer.class);
		job.setNumReduceTasks(50);
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Text.class);
		job.setOutputFormatClass(TextOutputFormat.class);
		if(inputPath == null){
			inputPath = MyMR.getConfigureValue(conf,"fs.path.reservedinference.inputs");
			FileInputFormat.addInputPaths(job, inputPath);
		}else
			FileInputFormat.addInputPath(job, new Path(inputPath));
		
		if(outputPath == null){
			outputPath = MyMR.getConfigureValue(conf,"fs.path.reservedinference.output");
		}
	...
	public static void main(String[] args) throws Exception{
		Tool tool = new ReservedInference();
		Configuration conf = HBaseConfiguration.create();
		Main.getConf(conf, args);
		System.out.println("Reserved Inference configuration:" + MyMR.getConfigureValue(conf,"reservedinference.classes"));
